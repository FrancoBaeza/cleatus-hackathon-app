...
export class AgentLogger {
  ...
  static logAgentSuccess(agent: string, action: string, startTime: string, outputData?: any) {
  ...
  static logAgentError(agent: string, action: string, startTime: string, error: any) {
    const endTime = new Date().toISOString();
    const duration = new Date(endTime).getTime() - new Date(startTime).getTime();
  ...
  static getLogsSummary() {
    const summary = {
      ...
      agentStats: {} as Record<string, { calls: number; avgDuration: number; errors: number }>
    };

    // Calculate agent statistics
    this.logs.forEach(log => {
      if (!summary.agentStats[log.agent]) {
        summary.agentStats[log.agent] = { calls: 0, avgDuration: 0, errors: 0 };
      }
