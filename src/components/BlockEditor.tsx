...
const getBlockIcon = (type: BlockType) => {
  switch (type) {
    case 'H1': return <Hash className="w-4 h-4" />;
    case 'H2': return <Hash className="w-3 h-3" />;
    case 'H3': return <Hash className="w-2 h-2" />;
        ...
        <div key={field.id} className="p-3 border rounded-lg bg-gray-50">
          <div className="grid grid-cols-2 gap-2 mb-2">
            ...
            <select
              ...
              value={field.type}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => updateField(index, { type: e.target.value as any })}
            >
          ...
          <div className="grid grid-cols-2 gap-2">
            <Input
              ...
              value={field.value}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateField(index, { value: e.target.value })}
            />
            <Input
              placeholder="Placeholder"
              value={field.placeholder || ''}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateField(index, { placeholder: e.target.value })}
            />
          </div>
        </div>
      ))}
      ...
      return (
        <div className="space-y-4">
          <Input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditedBlock({ ...editedBlock, title: e.target.value })}
          />
          
          {block.type === 'Form' && editedBlock.metadata?.formFields ? (
            <div>
            ...
            <Textarea
              ...
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setEditedBlock({ ...editedBlock, content: e.target.value })}
              rows={6}
            />
          )}
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className={`${getBlockStyle()}`}>{block.title}</span>
          </div>
...
Generated by CLEATUS AI Agent 2.0 - Confidence Score: ${confidenceScore}%`;
  ...
  const exportResponse = () => {
    const responseContent = blocks
      ...
      .map(block => {
        if (block.type === 'H1') return `# ${block.title}\n\n${block.content}`;
        if (block.type === 'H2') return `## ${block.title}\n\n${block.content}`;
        if (block.type === 'H3') return `### ${block.title}\n\n${block.content}`;
        if (block.type === 'Form') return `## ${block.title}\n\n[FORM: ${block.content}]`;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl">RFQ Response Editor</CardTitle>
              <p className="text-gray-600 mt-1">
                Edit your generated response blocks. Drag to reorder, click edit to modify content.
              </p>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <Button onClick={exportResponse} variant="outline" size="sm">
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <p className="text-2xl font-bold">
                {blocks.filter(b => b.type.startsWith('H')).length}
              </p>
